...
const PluginLoader = class PluginLoader {
	...
	get pluginNames(){
		return Object.keys(this.plugins).sort((a, b) => this.plugins[a].priority - this.plugins[b].priority);
	}
				...
				return new Promise((res, rej) => {
					...
					fs.stat(pluginPath, (err, stats) => {
						if (err) {
							...
							return res({success: false, plugin: null});
						};
